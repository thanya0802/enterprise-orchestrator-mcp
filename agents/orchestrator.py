...
writer_agent should depend on all other agents."""
    ...
    try:
        ...
        # Rate limit persisted or non-retryable error. Multi-agent mode
        # makes several LLM calls per question, so this is the most likely
        # spot to hit a free-tier RPM ceiling — degrade to a single
        # writer_agent pass over whatever context exists rather than crash.
    ...
    return parse_json_response(response.text or "[]", default=[])
